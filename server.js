...
async function sendTransactionalEmail({ to, subject, textBody, htmlBody, previewLink = '' }) {
  const payload = {
    from: process.env.SMTP_FROM || 'ZIA Flow <no-reply@ziaflow.local>',
    to,
    subject,
    text: textBody,
    html: htmlBody
...
function buildInviteEmail(user, link) {
  const subject = 'Invitación a ZIA Flow';
  const textBody = `Hola ${user.name || 'equipo'},\n\nTe invitaron a ZIA Flow. Activa tu cuenta aquí:\n${link}\n\nEste enlace vence en 72 horas.`;
  const htmlBody = `<p>Hola <strong>${user.name || 'equipo'}</strong>,</p><p>Te invitaron a <strong>ZIA Flow</strong>. Activa tu cuenta aquí:</p><p><a href="${link}">${link}</a></p><p>Este enlace vence en 72 horas.</p>`;
  return { subject, textBody, htmlBody };
}

function buildResetEmail(user, link) {
  const subject = 'Recuperación de contraseña de ZIA Flow';
...
Se te asignó una tarea en ZIA Flow.
...
Estas son tus tareas abiertas de hoy en ZIA Flow:

${lines.join('\n')}

Abre tu panel: ${link}`;
  const htmlBody = `<p>Hola <strong>${user.name || 'equipo'}</strong>,</p><p>Estas son tus tareas abiertas de hoy en <strong>ZIA Flow</strong>:</p><ul>${listHtml}</ul><p><a href="${link}">Abrir ZIA Flow</a></p>`;
  return { subject, textBody, htmlBody, previewLink: link };
}
...
app.use((error, _req, res, _next) => {
  console.error('ZIA Flow error:', error);
...
(async () => {
  ...
  app.listen(PORT, () => {
    console.log(`ZIA Flow running on ${APP_BASE_URL} · storage=${adapter.kind}`);
  });
})();
