...
function defaultSeedData() {
  ...
  const adminEmail = String(process.env.SEED_ADMIN_EMAIL || process.env.SMTP_USER || 'admin@zialabagency.com').trim().toLowerCase();
  return {
...
async function sendTransactionalEmail({ to, subject, textBody, htmlBody, previewLink = '' }) {
  const payload = {
    from: process.env.SMTP_FROM || 'Zia WorkSpace <no-reply@zialabagency.com>',
    to,
    subject,
    text: textBody,
    html: htmlBody
...
Abre tu panel aquí: ${link}`;
  const htmlBody = `<p>Hola <strong>${user.name || 'equipo'}</strong>,</p><p>Se te asignó una tarea en <strong>Zia WorkSpace</strong>.</p><ul><li><strong>Tarea:</strong> ${task.title}</li><li><strong>Cliente:</strong> ${clientName}</li><li><strong>Estado:</strong> ${task.status}</li><li><strong>Fecha límite:</strong> ${task.dueDate || 'Sin fecha'}</li></ul><p><a href="${link}">Abrir Zia WorkSpace</a></p>`;
